...
write_file Dockerfile <<'STUB_EOF_Dockerfile'
FROM ggoitea/laravel-dev:13-php8.5
...
USER app
STUB_EOF_Dockerfile
write_file docker-compose.yml <<'STUB_EOF_docker_compose_yml'
version: '3.8'
...
networks:
  ipvu:
    driver: bridge
STUB_EOF_docker_compose_yml
write_file Makefile <<'STUB_EOF_Makefile'
...
%:
	@:
STUB_EOF_Makefile
write_file devops.sh <<'STUB_EOF_devops_sh'
#!/usr/bin/env sh
...
STUB_EOF_devops_sh
write_file init.sh <<'STUB_EOF_init_sh'
#!/bin/sh
...
npm install
STUB_EOF_init_sh
write_file docker/Dockerfile.dev <<'STUB_EOF_docker_Dockerfile_dev'
FROM ggoitea/laravel-dev:13-php8.5
...
USER app
STUB_EOF_docker_Dockerfile_dev
write_file docker/Dockerfile.prod <<'STUB_EOF_docker_Dockerfile_prod'
FROM ggoitea/laravel-prod:13-php8.5
...
STUB_EOF_docker_Dockerfile_prod
write_file docker/nginx/default.conf <<'STUB_EOF_docker_nginx_default_conf'
upstream php_fpm {
    server app:9000;
}
...
STUB_EOF_docker_nginx_default_conf
write_file docker/php/php.ini-development <<'STUB_EOF_docker_php_php_ini_development'
[PHP]
...
;ffi.preload=
STUB_EOF_docker_php_php_ini_development
write_file docker/php/php.ini-production <<'STUB_EOF_docker_php_php_ini_production'
[PHP]
...
; List of headers files to preload, wildcard patterns allowed.
;ffi.preload=
STUB_EOF_docker_php_php_ini_production
write_file .dockerignore <<'STUB_EOF__dockerignore'
.git
.vscode
.agents
.cursor
